...
// Inlined from supabase/functions/_shared/zenoho_analysis_markers_prompt_v1_0.js
// Source: zenoho_analysis_markers_prompt_v1_0 v1.0 — Stage 1 of 3
const SYSTEM_PROMPT = `
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const concisenessConstraint = `\n\n---\nOUTPUT FORMAT CONSTRAINT (CRITICAL):\nYour JSON response MUST fit within 8000 output tokens. To achieve this:\n1. Limit each marker's "narrative" field to ONE short sentence, maximum 20 words.\n2. Limit "interpretation_note" to ONE short sentence or null for optimal markers.\n3. Limit "physician_referral" to ONE sentence maximum.\n4. For markers in optimal zone, set narrative to "Within Zenoho Optimal range." and physician_referral to null.\n5. Keep all required schema fields. Only prose fields should be terse.\nTruncated JSON causes parse failure. Be concise.\n---`;
...
Return Stage 1 JSON only.${concisenessConstraint}`;
