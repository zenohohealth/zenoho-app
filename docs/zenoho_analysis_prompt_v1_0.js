...
# ============================================================
# HOW TO USE IN BOLT SERVER FUNCTION
# ============================================================
# 1. Import SYSTEM_PROMPT from this file
# 2. Convert user's uploaded PDF to base64
# 3. Call Claude API with PDF as document + "Analyse this report" as user message
# 4. Parse JSON from response content
# 5. Store results in Supabase using zenoho_database_schema_v1_0.sql tables
#
# Example Bolt server function:
#
...

{
  ...
  "biological_age": {
    ...
    "bio_age_gap": number,
    "bio_age_score": number,
    "bio_age_level": number,
    "confidence": "HIGH" | "MEDIUM" | "LOW",
  ...
  "domain_scores": [
    {
      ...
      "headline": "string (max 15 words, performance language)"
    }
  ],
  ...
  "safety_overrides_triggered": [
    {
      ...
      "physician_urgency": "24_hours" | "48_hours" | "1_week"
    }
  ],
  ...
  "supplement_protocol": [
    {
      ...
      "rationale_marker_ids": [number],
      "retest_at_weeks": number
    }
  ],
  ...
  "zenoho_read_summary": {
    "headline_score": "string (e.g. Level 7 — Healthy Performance)",
    "confidence_note": "string",
    "top_3_findings": ["string", "string", "string"],
    "top_3_opportunities": ["string", "string", "string"],
    "next_test_recommendation": "string",
    "closing_note": "string (max 30 words, forward-looking, performance language)"
  }
}
...
- If pregnant (noted in report): add "pregnancy_detected": true to panel_metadata, 
  skip hormone domains, add note to biological_age that BioAge is not calculated
- If report is from outside India and units differ significantly: convert to Indian units 
  and note the conversion
- If the same marker appears on multiple accessions with different values: 
  use the value from the accession with matching sample type (serum > whole blood > calculated)
- If a value is flagged as haemolysed/lipaemic in the report: set retest_required=true, 
  zenoho_zone=null, lab_error_flag="haemolysis" or "lipaemia"
...
// ============================================================
/*
import { SYSTEM_PROMPT } from './zenoho_analysis_prompt.js'
        ...
        content: [
          ...
          },
          {
            ...
            text: `Analyse this blood report. Patient profile for context:
              Age: ${userProfile.age}
              Sex: ${userProfile.sex}
              Dietary pattern: ${userProfile.dietary_pattern}
              Return JSON only.`
          }
        ]
      }]
